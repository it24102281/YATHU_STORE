...
router.post('/signup', async (req, res) => {
  try {
    ...
    try {
      ...
      // Clean up the user account since verification email could not be delivered
  ...
  try {
    ...
    if (!user.isEmailVerified) {
      return res.status(401).json({
        success: false,
        message: 'Your email address is not verified. Please verify your email or sign up again.',
      });
    }
